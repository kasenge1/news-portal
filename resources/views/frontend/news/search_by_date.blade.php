...
<a href=" "><i class="las la-home"></i> </a> <i class="las la-chevron-right"></i> Search For Date: {{$dateFormat}} 
</div>
...
@foreach($newnews as $key => $item)
<div class="archive-tabWrpp archiveTab-border">
<div class="archiveTab-image ">
<a href="{{url('news/details/'.$item->id.'/'.$item->news_slug_title)}} "><img class="lazyload" src="{{(!empty($item->image))? asset($item->image): url('upload/no_image.jpg')}}"  ></a> </div>
<a href=" " class="archiveTab-icon2"><i class="la la-play"></i></a>
<h4 class="archiveTab_hadding"><a href=" {{url('news/details/'.$item->id.'/'.$item->news_slug_title)}}">{{$item->news_title}} </a>
</h4>
