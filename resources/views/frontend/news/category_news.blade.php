...
@section('title')
{{$breadcrumb->category_name}} |online news
@endsection
...
<a href=" "><i class="las la-home"></i> </a> <i class="las la-chevron-right"></i> {{$breadcrumb->category_name}}
</div>
<div class="row">
...
<a href="{{url('news/details/'.$item->id.'/'.$item->news_slug_title)}} "><img class="lazyload" src="{{(!empty($new->image))? asset($new->image):url('upload/no_image.jpg')}}"  ></a> </div>
<a href=" " class="archiveTab-icon2"><i class="la la-play"></i></a>
<h4 class="archiveTab_hadding"><a href=" ">{{$new->news_title}} </a>
</h4>
