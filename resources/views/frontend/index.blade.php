...
$topslider = App\Models\Newspost::where('status',1)->where('top_slider',1)->limit(7)->get();
...
<a href="{{url('news/details/'.$slider->id.'/'.$slider->news_slug_title)}} "><img class="lazyload" src="{{asset($slider->image)}}"></a>
...
<a href=" {{url('news/details/'.$slider->id.'/'.$slider->news_slug_title)}}">{{$slider->news_title}}</a>
</h1>
</div>
</div>
</div>
...
<div class="owl-nav"><button type="button" role="presentation" class="owl-prev"><i class="fa-solid fa-angle-left"></i></button>
...
$threesection = App\Models\Newspost::where('status',1)->where('first_section_three',1)->limit(3)->get();
...
<a href="{{url('news/details/'.$section->id.'/'.$section->news_slug_title)}} "><img class="lazyload" src="{{asset($section->image)}}"  ></a>
<h5 class="secOne_smallTitle">
<a href=" {{url('news/details/'.$section->id.'/'.$section->news_slug_title)}}">{{$section->news_title}} </a>
</h5>
</div>
</div>
@endforeach
...
$ninesection = App\Models\Newspost::where('status',1)->where('first_section_nine',1)->latest()->limit(9)->get();

@endphp

@foreach($ninesection as $section)
<div class="themesBazar-3 themesBazar-m2">
<div class="sec-one-wrpp2">
<div class="secOne-news">
<div class="secOne-image2">
<a href=" "><img class="lazyload" src="{{asset($section->image)}}"  ></a>
</div>
...
@php
    $live = App\Models\Livetv::findOrFail(1);
@endphp
...
<img width="700" height="400" src="{{(!empty($live->live_image))?asset($live->live_image): url('upload/no_image.jpg')}}" class="attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="" loading="lazy">
...
<iframe width="560" height="315" src="{{$live->live_url}}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
</div>
</div>
</div>
</div> 
...
<a href=" {{url('news/details/'.$latest->id.'/'.$latest->news_slug_title)}}"><img class="lazyload" src="{{(!empty($latest->image))? asset($latest->image): url('upload/no_image.jpg')}}"  ></a>
<a href=" " class="tab-icon"><i class="la la-play"></i></a>
<h4 class="tab_hadding"><a href=" {{url('news/details/'.$latest->id.'/'.$latest->news_slug_title)}}">{{$latest->news_title}} </a></h4>
</div>
@endforeach
</div>
</div>
...
<div class="tab-image tab-border">
<a href="{{url('news/details/'.$popular->id.'/'.$popular->news_slug_title)}} "><img class="lazyload" src="{{(!empty($popular->image))? asset($popular->image): url('upload/no_image.jpg')}}"  ></a>
<a href=" " class="tab-icon"><i class="la la-play"></i></a>
<h4 class="tab_hadding"><a href="{{url('news/details/'.$popular->id.'/'.$popular->news_slug_title)}} ">{{$popular->news_title}}</a></h4>
</div>
@endforeach
...
$news = App\Models\Newspost::where('status',1)->orderBy('id','ASC')->latest()->limit(8)->get();
...
<li class="nav-item" role="presentation">
<div class="nav-link" id="categori-tab2" data-bs-toggle="pill" data-bs-target="#category{{$item->id}}" role="tab" aria-controls="Info-tabs2" aria-selected="false">
...
<a href="{{ url('news/details/'.$post->id.'/'.$post->news_slug_title)}}"><img class="lazyload" src="{{(!empty($post->image))? asset($post->image):url('upload/no_image.jpg')}}"  ></a>
</div>
<h5 class="sec-two-title">
<a href="{{ url('news/details/'.$post->id.'/'.$post->news_slug_title)}} ">{{$post->news_title}} </a>
</h5>
</div>
</div>
@endforeach
...
$catnews = App\Models\Newspost::where('category_id',$category->id)->orderBy('id','DESC')->limit(8)->get();
...
<a href="{{ url('news/details/'.$item->id.'/'.$item->news_slug_title)}}">{{$item->news_title}} </a>
</h4>
</div>
@endif
@endforeach
...
@php
    $photo = App\Models\Photogallery::latest()->get();
@endphp
...
@php
    $video = App\Models\Videogallary::latest()->limit(6)->get();
@endphp
@foreach($video as $item)

<div class="secFive-smallItem">
<div class="secFive-smallImg">
<img src="{{(!empty($item->video_image))? asset($item->video_image): url('upload/no_image.jpg')}}">
<a href="{{$item->video_url}}" class="home-video-icon popup"><i class="las la-video"></i></a>
<h5 class="secFive_title2">
<a href="{{$item->video_url}}" class="popup"> {{$item->video_title}} </a>
</h5>
</div>
</div>
@endforeach
